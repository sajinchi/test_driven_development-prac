...
import { DeleteInvoiceService } from '@/src/services/invoice/invoiceDelete.service';

const Delete = (props:{id:string}) => {
  const [deleteOpen, setDeleteOpen] = useState(false);

  const deleteInvoice = async (id: string) => {
    const response: INetwortRequestResponseState = await DeleteInvoiceService(id);
              ...
              >
                <Dialog.Panel className="w-full max-w-xl transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                  ...
                  <div className="flex flex-row p-5 space-x-2 justify-center">
                    ...
                    <button
                      onClick={() => deleteInvoice(props.id)}
